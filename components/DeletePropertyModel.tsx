...
import { deleteUserProperty } from "@/lib/services/property.service";
...
export function ConfirmDeleteModal({
  ...
  if (!isOpen || !propertyId || !mounted) return null;

  const handleDelete = async (id: string) => {
    setIsDeleting(true);
    const toastId = toast.loading('Deleting the property...');
    try {
      const res = await deleteUserProperty(id);
      if (!res.success) {
        toast.error(res.error || 'Failed to delete property', { id: toastId });
        return;
      }
      toast.success(res.message || 'Property deleted successfully', { id: toastId });
      onClose();
      router.refresh();
    } catch (error) {
